...
def refund_payment(payment_id):
    ...
    try:
        ...
        # Process the refund
        return process_refund_transaction(payment, refund_amount)

    except Exception as e:
        db.session.rollback()
        return jsonify({'error': str(e)}), 500

def process_refund_transaction(payment, refund_amount):
    """Helper function to process the actual refund transaction"""
    ...
    rewards = Reward.query.filter_by(payment_id=payment.id).all()
